...
if __name__=='__main__':
    ...
    cdataloader = DataLoader(coco,collate_fn=coco.batch_collator,batch_size=2,shuffle=True)

    for i,d in enumerate(cdataloader):
        if i>=10:
            break
        bn = 1
        img = (d['raw']['img'][bn]*255).cpu().numpy().squeeze().astype(np.int).astype(np.uint8)
        img_warp = (d['warp']['img'][bn]*255).cpu().numpy().squeeze().astype(np.int).astype(np.uint8)
        img = cv2.merge([img, img, img])
        img_warp = cv2.merge([img_warp, img_warp, img_warp])

        kpts = np.where(d['raw']['kpts_map'][bn].squeeze().cpu().numpy())
        kpts = np.vstack(kpts).T
        kpts = np.round(kpts).astype(np.int)
        for kp in kpts:
            cv2.circle(img, (kp[1], kp[0]), radius=3, color=(0,255,0))
        kpts = np.where(d['warp']['kpts_map'][bn].squeeze().cpu().numpy())
        kpts = np.vstack(kpts).T
        kpts = np.round(kpts).astype(np.int)
        for kp in kpts:
            cv2.circle(img_warp, (kp[1], kp[0]), radius=3, color=(0,255,0))

        mask = d['raw']['mask'][bn].cpu().numpy().squeeze().astype(np.int).astype(np.uint8)*255
        warp_mask = d['warp']['mask'][bn].cpu().numpy().squeeze().astype(np.int).astype(np.uint8)*255

        # unzero = (img_warp[:,:,0]!=0)
        # img_warp[:, :, 0][unzero] = 255
        # img_warp[:, :, 1][unzero] = 0
        # img_warp[:, :, 2][unzero] = 0
        #
        # warp_mask = warp_mask.astype(np.bool)
        # img_warp[:, :, 0][warp_mask] = 0
        # img_warp[:, :, 1][warp_mask] = 0
        # img_warp[:, :, 2][warp_mask] = 255
        #
        # cv2.imwrite('debug.png', img_warp)
        # #cv2.imshow('debug',img_warp)
        # #cv2.waitKey()
        # plt.imshow(img_warp)
        # plt.show()
